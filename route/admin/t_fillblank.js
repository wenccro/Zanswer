...
exports.showIndex=function(req,res){
	...
	var promise=new Promise(function(resolve, reject){
		db.query(`SELECT course_table.c_name,chapter_table.ch_num,test_fillblank_table.ID,test_fillblank_table.t_title FROM chapter_table,course_table,test_fillblank_table where chapter_table.c_ID = course_table.ID and test_fillblank_table.ch_ID=chapter_table.ID and chapter_table.ch_num!=0 and test_fillblank_table.t_title!='null' limit `+i+`,10`,(err,data)=>{
			if(err){
				console.log(err);
...
exports.addPage=function(req,res){
	res.render('admin/test_fill_add.ejs',{});
}
//用来应付window.location重置的路径
exports.alterPage=function(req,res){
	res.render('admin/test_fill_alter.ejs',{});
}
...
exports.addIndex=function(req,res){
	...
	db.query(`INSERT INTO test_fillblank_table(t_title,t_answer,t_explain,blank_count,ch_ID) VALUE('${t_title}','${t_answer}','${t_explain}','${blank_count}','${ch_ID}')`,(err,data)=>{
		if(err){
			console.log(err);
			res.status(500).send("database error").end();
		}else{
...
exports.deleteTest=function(req,res){
	...
	//console.log(t_id);
	db.query(`DELETE FROM test_fillblank_table WHERE ID=${t_id}`,(err,data)=>{
		if(err){
			console.error(err);
			res.status(500).send('database error').end();
		}else{
...
exports.alterShow=function(req,res){
	...
	//先拿到标题
	var promise=new Promise(function(resolve,reject){
		db.query(`SELECT chapter_table.ch_name,course_table.c_name from chapter_table,course_table,test_fillblank_table where test_fillblank_table.ID='${t_id}' and test_fillblank_table.ch_ID=chapter_table.ID and chapter_table.c_ID=course_table.ID`,(err,data)=>{
			if(err){
				console.error(err);
				res.status(500).send('database error').end();
			}else{
		...
		return new Promise(function(resolve,reject){
			db.query(`SELECT * FROM test_fillblank_table WHERE ID='${t_id}'`,(err,data)=>{
				if(err){
					console.error(err);
					res.status(500).send('database error').end();
				}else{
...
exports.alterIndex=function(req,res){
	...
	db.query(`UPDATE test_fillblank_table SET t_title='${t_title}',t_answer='${t_answer}',t_explain='${t_explain}' where ID='${t_id}'`,(err,data)=>{
		if(err){
			cosele.error(err);
			res.status(500).send('database error').end();
		}else{
